...
export default function CrossVerifyTable({ results }: CrossVerifyTableProps) {
  return (
    <div className="overflow-hidden bg-card shadow-card sm:rounded-card border border-border">
            ...
            <tr key={idx} className="hover:bg-surface transition-colors">
              ...
              <td className="whitespace-nowrap px-6 py-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  {item.status === 'PENDING' && <div className="h-4 w-4 rounded-full border-2 border-border border-t-primary animate-spin" />}
                  <span className={`font-semibold ${
                    item.status === 'VERIFIED' ? 'text-success-text' :
                    item.status === 'MISMATCH' ? 'text-danger-text' :
                    item.status === 'NOT_FOUND' ? 'text-warning-text' : 'text-muted'
                  }`}>
                    {item.status}
                  </span>
                </div>
              </td>
              <td className="px-6 py-4 text-sm text-muted truncate max-w-xs" title={item.message}>
                {item.message}
              </td>
            </tr>
          ))}
